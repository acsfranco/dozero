...
// Criando a tabela de descritores de interrupção (IDT)

idt_entry_t IDT[256];
idtr_t idtr, *idtp;

void idt_set_gate(uint8_t intnum, uint32_t handle, uint16_t selector, uint8_t flags) {
  IDT[intnum].offset_lowerbits = (handle & 0xffff);
  IDT[intnum].selector = selector;
  IDT[intnum].zero = 0;
  IDT[intnum].type_attr = flags;
  IDT[intnum].offset_higherbits = (handle >> 16) & 0xffff;
}
